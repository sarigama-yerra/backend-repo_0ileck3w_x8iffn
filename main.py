...
app = FastAPI(title="Rent2Rent API", version="1.0.0")
...
@app.get("/")
def read_root():
    return {"message": "Rent2Rent backend is running"}
...
# Public endpoint to list a few recent inquiries (for admin preview/demo)
@app.get("/api/inquiries", response_model=List[dict])
def list_inquiries(limit: int = 10):
    try:
